...
RunPipelineOutput: Literal[0, 1]


def run_pipeline(config_file_path: str) -> Literal[0, 1]:
...
def main():
    if len(sys.argv) < 2:
        sys.stderr.write(
            ...
            "💡 Usage:\n"
            "   uv run python -m src.main <CONFIG_PATH>\n\n"
        )
        sys.exit(1)

    CONFIG_INPUT = sys.argv[1]

    return_code = run_pipeline(CONFIG_INPUT)
